...
function LoginForm() {
  ...
  return (
    ...
    >
      <Link
        href="/"
        className="absolute left-4 top-4 rounded-lg border border-brand-300 bg-surface/80 px-3 py-1.5 text-[19px] text-slate-600 backdrop-blur transition hover:text-slate-900"
      >
        ← Home
      </Link>
      <div className="card-themed w-full max-w-sm rounded-2xl border border-brand-300 p-8 shadow-sm">
        ...
        {step === "email" && (
          <form action={requestAction} className="mt-6 space-y-4">
            ...
            {requestState.error && (
              <p className="text-[19px] text-red-600">{requestState.error}</p>
            )}
        ...
        {step === "code" && (
          <form action={verifyAction} className="mt-6 space-y-4">
            ...
            <div>
              <label className="block text-[19px] font-medium text-slate-700">
                Sign-in code
              </label>
              <input
                name="code"
            ...
            {verifyState.error && (
              <p className="text-[19px] text-red-600">{verifyState.error}</p>
            )}
            <SubmitButton label="Verify & sign in" />
            <button
              type="button"
              onClick={() => setStep("email")}
              className="w-full text-center text-[19px] text-slate-500 hover:text-slate-700"
            >
